...
double calculate_air_density(double altitude) {
    ...
    if (altitude > 150000) return 0.0;
    return RHO_0 * exp(-altitude / H_SCALE);
}


double calculate_gravity_mag(double r) {
...
static double calculate_isp(double altitude) {
    if (altitude >= 50000) return ISP_VACUUM;
    if (altitude <= 0)     return ISP_SEA_LEVEL;
 ...
 */
 ...
 void calculate_forces(State *state, double pitch, double throttle, Forces *forces) {
    ...
    if (horiz_len > 1.0) {
        ut_x = -rz/r;  ut_y = 0.0;  ut_z = rx/r;
    } else {
        // Degenerate case (rocket directly over pole) – use +x
        ut_x = 1.0; ut_y = 0.0; ut_z = 0.0;
    }
...
void runge_kutta_step(State *state, double pitch, double throttle) {
    ...
    #define ADVANCE(base, kk, dt_scale, out) do {              \
    ...
    if (state->mass < DRY_MASS) state->mass = DRY_MASS;
}
